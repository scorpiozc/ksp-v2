	...
	public void loadSectionData() {
		...
		try (BufferedReader reader = readFile(CalcConstant.LINE_INFO);) {
			...
			while (true) {
				...
				if (stations[s1].getLine() != null && stations[s1].getLine().getCode() != lineID)
					throw new IllegalArgumentException("Input first station is conflicting.");
				if (stations[s2].getLine() != null && stations[s2].getLine().getCode() != lineID)
					throw new IllegalArgumentException("Input first station is conflicting.");

				// set the tracks that they are on
				stations[s1].setLine(lines[lineID]);
				stations[s2].setLine(lines[lineID]);

				// create a new Line
				int traveltime = time + (parktimes.get(s1 + "-" + s2) == null ? 0 : parktimes.get(s1 + "-" + s2));
				double impedance = traveltime;
				Section connect = new Section(stations[s1], stations[s2], lines[lineID], lines++, dir, dist,
						traveltime, impedance);
//				System.out.printf("s1:%s-s2:%s\n", s1, s2);
				stations[s1].addSection(connect);
				stations[s2].addSection(connect);
			}
		} catch (IOException e) {
			logger.error(e.getMessage(), e);
		}
